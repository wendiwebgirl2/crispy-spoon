...
export function generateVideo(script, { token = currentToken(), title, avatarId, caption, background, aspectRatio, backgroundAssetId, engine, expressiveness, motionPrompt, deliveryLabel, deliveryPrompt, backgroundFile } = {}) {
  return apiPostJson("/api/videos/generate", { token, script, title, avatar_id: avatarId, caption: !!caption, background: background || null, aspect_ratio: aspectRatio || null, background_asset_id: backgroundAssetId || null, engine: engine || null, expressiveness: expressiveness || null, motion_prompt: motionPrompt || null, delivery_label: deliveryLabel || null, delivery_prompt: deliveryPrompt || null, background_file: backgroundFile || null });
}
...
export const api = {
  ...
  listRecordings: (token) => apiGet("/api/recordings/" + encodeURIComponent(token)),
  deleteAvatar: (token, id) => apiDelete("/api/avatars/" + encodeURIComponent(token) + "/" + encodeURIComponent(id)),
  listVideos: (token) => apiGet("/api/videos/" + encodeURIComponent(token)),
  deleteRecording: (recId, token) => apiDelete("/api/recordings/" + encodeURIComponent(token) + "/" + encodeURIComponent(recId)),
  ...
  verifyCredential2fa: (id, credId, verified, name) => vcReq(`/clients/${id}/credentials/${credId}/twofa`, { method: "PUT", body: JSON.stringify({ verified, name }) }),
  getContract: (id) => vcReq(`/clients/${id}/contract`),
  saveContract: (id, payload) => vcReq(`/clients/${id}/contract`, { method: "PUT", body: JSON.stringify(payload) }),
  sendContract: (id, email) => vcReq(`/clients/${id}/contract/send`, { method: "POST", body: JSON.stringify({ email: email || "" }) }),
  voidContract: (id) => vcReq(`/clients/${id}/contract/void`, { method: "POST", body: "{}" }),
  countersignContract: (id, name) => vcReq(`/clients/${id}/contract/countersign`, { method: "POST", body: JSON.stringify({ name }) }),
  contractPdfUrl: (id) => `${VC_BASE}/clients/${id}/contract/pdf`,
  getOnboarding: (id) => vcReq(`/clients/${id}/onboarding`),
  updateOnboardingTask: (id, taskId, payload) => vcReq(`/clients/${id}/onboarding/${taskId}`, { method: "PUT", body: JSON.stringify(payload) }),
  addOnboardingTask: (id, label, assignee) => vcReq(`/clients/${id}/onboarding`, { method: "POST", body: JSON.stringify({ label, assignee: assignee || undefined }) }),
  deleteOnboardingTask: (id, taskId) => vcReq(`/clients/${id}/onboarding/${taskId}`, { method: "DELETE" }),
  setOnboardingBookingUrl: (id, url) => vcReq(`/clients/${id}/onboarding/settings/booking-url`, { method: "PUT", body: JSON.stringify({ url }) }),
  ...
  metaStatus: (id) => vcReq(`/clients/${id}/distribution/meta-status`),
  channelStatus: (id, channel) => vcReq(`/clients/${id}/distribution/channel-status/${channel}`),
  ...
  regenerateUserPassword: (uid, email, sendEmail) => vcReq(`/users/${uid}/regenerate`, { method: 'POST', body: JSON.stringify({ email, sendEmail }) }),
  deleteUser: (uid) => vcReq(`/users/${uid}`, { method: 'DELETE' }),
  impersonateUser: (uid) => vcReq(`/users/${uid}/impersonate`, { method: 'POST' }),
  notifyUser: (uid, message) => vcReq(`/users/${uid}/notify`, { method: 'POST', body: JSON.stringify({ message }) }),
  changeMyPassword: (currentPassword, newPassword) => vcReq('/me/password', { method: 'POST', body: JSON.stringify({ currentPassword, newPassword }) }),
  myNotifications: () => vcReq('/me/notifications'),
  dismissNotification: (id) => vcReq(`/me/notifications/${id}/read`, { method: 'POST' }),
  ...
  portalAvatarLooks: (avatarId) => vcReq(`/portal/avatars/${encodeURIComponent(avatarId)}/looks`),
  portalSetAvatarLook: (avatarId, heygenAvatarId, imageUrl) => vcReq(`/portal/avatars/${encodeURIComponent(avatarId)}/set-look`, { method: 'POST', body: JSON.stringify({ heygen_avatar_id: heygenAvatarId, image_url: imageUrl || null }) }),
  ...
  portalMetaLink: () => vcReq('/portal/onboarding/meta-link', { method: 'POST', body: JSON.stringify({}) }),
  portalChannelLink: (channel) => vcReq(`/portal/onboarding/channel-link/${channel}`, { method: 'POST', body: JSON.stringify({}) }),
